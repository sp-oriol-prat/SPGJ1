...
public class UIAtlasMaker : EditorWindow
{
    ...
    Dictionary<string, int> GetSpriteList (List<Texture> textures)
    {
        ...
        if (NGUISettings.atlas != null && NGUISettings.atlas.name == atlasName)
        {
            ...
            for(int k = 0; k < spriteNames.size; k++)
            {
                string sp = spriteNames[k];
                spriteList.Add(sp, 0);
            }
        }
        ...
        if (textures.Count > 0)
        {
            ...
            for(int k = 0; k < texNames.Count; k++)
            {
                string tex = texNames[k];
    ...
    static public void ExtractSprites (UIAtlas atlas, List<SpriteEntry> finalSprites)
    {
        ...
        if (tex != null)
        {
            ...
            for(int k = 0; k < sprites.Count; k++)
            {
                UISpriteData es = sprites[k];
                ShowProgress((index++) / count);

                bool found = false;

                for(int z = 0; z < finalSprites.Count; z++)
                {
                    SpriteEntry fs = finalSprites[z];
                    if (es.name == fs.name)
                    {
    ...
    void OnGUI ()
    {
        ...
        if (spriteList.Count > 0)
        {
            NGUIEditorTools.DrawHeader("Sprites", true);
            {
                ...
                {
                    KeyValuePair<string, int> iter = spriteList.ElementAt(k);
                    ++index;
