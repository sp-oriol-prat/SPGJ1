...
public class NGUIJson
{
    ...
    public static void LoadSpriteData (UIAtlas atlas, TextAsset asset)
    {
        ...
        object[] keys = new object[frames.Keys.Count];
        frames.Keys.CopyTo(keys, 0);

        for(int k = 0; k < keys.Length; k++)
        {
            object key = keys[k];
            object value = frames[key];

            UISpriteData newSprite = new UISpriteData();
            newSprite.name = key.ToString();

            bool exists = false;

            // Check to see if this sprite exists
            for(int z = 0; z < oldSprites.Count; z++)
            {
                UISpriteData oldSprite = oldSprites[z];
...
*/
    ...
    protected static bool serializeObject( Hashtable anObject, StringBuilder builder )
    {
        ...
        object[] keys = new object[anObject.Keys.Count];
        anObject.Keys.CopyTo(keys, 0);
        
        for(int k = 0; k < keys.Count(); k++)
        {
            string key = keys[k].ToString();
            object value = anObject[keys[k]];
    ...
    protected static bool serializeDictionary( Dictionary<string,string> dict, StringBuilder builder )
    {
        ...
        {
            KeyValuePair<string, string> kv = dict.ElementAt(k);
            if( !first )
                builder.Append( ", " );
